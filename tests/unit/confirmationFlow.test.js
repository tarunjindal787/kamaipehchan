...
async function run() {
  ...
  const res1 = mockRes();
  handleConfirmationReply({ body: { From: REPLY_1_WORKER, Body: '1' } }, res1);
  ...
  const res2 = mockRes();
  handleConfirmationReply({ body: { From: REPLY_2_WORKER, Body: '2' } }, res2);
  ...
  handleConfirmationReply({ body: { From: UNRECOGNIZED_WORKER, Body: 'maybe?' } }, resGarbage);
  ...
  // sent right after must still resolve it. With the original given code
  // (resolvePending called unconditionally before checking replyBody), this
  // would fail - the entry would already have been popped and discarded.
  const resValidFollowUp = mockRes();
  handleConfirmationReply({ body: { From: UNRECOGNIZED_WORKER, Body: '1' } }, resValidFollowUp);
  ...
  // --- 5. sendConfirmationPrompt with TWILIO_CONFIGURED false -> simulated
  ...
  const simResult = await sendConfirmationPrompt(
    { rail_id: 'rail_sim', worker_id: 'worker_sim_test', amount: 100000, note: '', credited_at: now },
    { label: 'needs_review', original_label: 'recurring_wage' }
  );
  ...
  console.log('✅ Unit test passed: confirmation flow (income filter, reply resolution, Twilio fallback) verified.');
}
