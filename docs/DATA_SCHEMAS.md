...
`reference_id` **does exist in a real webhook** - but only on `payment_link.paid`'s `payload.payment_link.entity.reference_id`. It is **never** present on `payload.payment.entity`, in either event type. **Wired directly (2026-09-01):** `normalize.js` now checks `payload.payment_link.entity.reference_id` first for `payment_link.paid` events specifically, before falling through to the notes-based composite (`notes.worker_id` + `notes.employer_ref`) that everything else (`payment.captured`, where the field is confirmed absent, and any other event type) still relies on. Confirmed against the real captured `payment_link.paid` payload (`tests/benchmark_payloads/real_test_mode/payment_link_paid_real_event.json`).

**New known gap surfaced by this fix, not yet resolved:** the notes-based composite produces a *differently formatted* string (`<workerId>_<employerRef>`, e.g. `worker_973712b5_ZEPTOTESTDEPLOY`) than the real `reference_id` (`RAIL_<workerId>_EMP_<SLUG>`, e.g. `RAIL_worker_973712b5_EMP_ZEPTOTESTDEPLOY`) for the exact same underlying rail. Since `transactionStore.js` groups history strictly by exact `rail_id` string equality, a rail whose transactions arrive as a mix of `payment_link.paid` (real `reference_id`, `RAIL_..._EMP_...` format) and `payment.captured` (composite fallback, `..._...` format) would silently fragment into two separate histories instead of one - breaking the deterministic classifier's rail-history matching and understating retention/regularity for that rail. Not yet fixed; worth reconciling the two formats (or reading `payload.payment_link.entity.reference_id` from `payment.captured` too, if it ever appears there) before this affects real scoring.
